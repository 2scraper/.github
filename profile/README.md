...
| [LG](https://www.lg.com) | [LG scraper](https://github.com/2scraper/lg-scraper) | Manufacturer catalogues for the RU & UA sites — one row per model group with every sibling screen size, sales model codes, model year, ratings, review counts, energy labels & where-to-buy links; these locales publish no prices, and the rows say so rather than writing the site's zeros as a price; the site's own catalogue API means no browser, and it runs with no key or proxy |
